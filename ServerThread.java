			...
			*/
			...
			try{
				...
				*/
			    ...
			    this.board.availableIDs[id]=true;        
				input.close();
                output.close();      
                socket.close();
			...
			while(true){
				...
				2) you've played and the game got over in this round.				?????????????? #done ig
				...
				*/
				...
				regardless of whether you're new.												??????????????	#ig done
				...
				*/
				...
				if (!client_quit){
					...
					//in case of IO Exception, off with the thread
						...
						//set flags 
						quit_while_reading=true; ////////???????????????    #ig done              
					...
					if (!indicator.equals("Play")){
						//Proceed simillarly to IOException					Recheck with IO exception
